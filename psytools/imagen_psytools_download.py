...
# The legacy service credentials are stored in the standard ~/.netrc file.
LEGACY_BASE_URL = 'https://www.delosis.com/psytools-server/dataservice/dataset/'
# The LSRC2 service is available from the same server, but a different URL.
...
LEGACY_CSV_DATASETS = (
    ...
    ('IMGN_NI_DATA_FU3', IMAGEN_DIGEST),  # (NI data entry)
    ('cVEDA_MINI5', BASIC_DIGEST),  # (M.I.N.I 5)
)

QUOTED_PATTERN = re.compile(r'".*?"', re.DOTALL)


def download_legacy(base_url, datasets):
...
def main():
    download_legacy(LEGACY_BASE_URL, LEGACY_CSV_DATASETS)
    download_lsrc2(LSRC2_BASE_URL)
