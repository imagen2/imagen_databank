...
def main():
    ...
    for datasheet in datasheets:
        ...
        if len(subject_ids) != 1:
            logger.warning('Proper "Subject ID" not found: {0}'
                           .format(datasheet))
            continue
        psc1 = subject_ids.pop()
        if psc1 not in PSC2_FROM_PSC1:
            logger.error('unknown PSC1 code {0}: {1}'.format(psc1, datasheet))
            continue
        psc2 = PSC2_FROM_PSC1[psc1]
        if psc2 not in DOB_FROM_PSC2:
            logger.error('unknown age for PSC2 code {0}: {1}'.format(psc2, datasheet))
            continue
        dob = DOB_FROM_PSC2[psc2]
        session_start_times = set([sst.date() for sst in session_start_times])
        if len(session_start_times) != 1:
            logger.warning('Proper "Session start time" not found: {0}'
                           .format(datasheet))
            continue
        session_start_time = session_start_times.pop()
        if session_start_time < date(2007, 1, 1):
            logger.error('Bogus "Session start time" {0}: {1}'
                         .format(session_start_time, datasheet))
            continue
